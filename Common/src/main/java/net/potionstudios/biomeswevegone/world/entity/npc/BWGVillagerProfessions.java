...
public class BWGVillagerProfessions {
    ...
    private static ResourceKey<VillagerProfession> register(String id, ResourceKey<PoiType> poiType, @Nullable SoundEvent soundEvent, ImmutableSet<Item> requestedItems){
        ResourceKey<VillagerProfession> name = BiomesWeveGone.key(Registries.VILLAGER_PROFESSION, id);
        PlatformHandler.PLATFORM_HANDLER.register(BuiltInRegistries.VILLAGER_PROFESSION, id, () -> new VillagerProfession(Component.translatable("entity." + name.location().getNamespace() + ".villager." + name.location().getPath()), poiTypeHolder -> poiTypeHolder.is(poiType), (poiTypeHolder) -> poiTypeHolder.is(poiType), requestedItems, ImmutableSet.of(), soundEvent));
        return name;
    }
